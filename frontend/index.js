...
document.querySelector('#citySelect').addEventListener('change', async evt => {
  ...
  try {
    ...
    .textContent = `${data.current.temperature_min} o/${data.current.temperature_max}o`
    document.querySelector('#todayStats div:nth-child(2)')
    .textContent = `precipilation: ${data.current.precipilation_probability * 100}%`
    ...
    data.forecast.daily.forEach((day, idx) => {
      ...
      minMax.textContent = `${day.temperature_min}o/${day.temperature_max}o`
      precipit.textContent = `precipitation: ${day.precipilation_probability * 100}%`
